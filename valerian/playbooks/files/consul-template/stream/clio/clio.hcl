job "clio" {
  ...
  group "clio" {
    ...
    task "clio" {
      ...
      config {
        image = "{{key "valerian 1.0/external services/valkyrie/address"}}/clio:{{key "valerian 1.0/versions/clio"}}"
        mounts = [
               {
                 type = "bind"
                 target = "/app/appsettings.json"
                 source = "${meta.DFS}/opt/clio/app/appsettings.json"
                },
                {
                 type = "bind"
                 target = "/app/Resources"
                 source = "${meta.DFS}/opt/clio/app/Resources"
                },
            	{
                 type = "bind"
                 target = "/app/logs"
                 source = "${meta.DFS}/logs/clio"
                },
                {
                    type = "bind"
                    target = "/etc/timezone"
      ...
      resources {
        cpu    = {{key "valerian 1.0/cluster config/resources/clio/cpu"}}
        memory = {{key "valerian 1.0/cluster config/resources/clio/memory"}}
      }
